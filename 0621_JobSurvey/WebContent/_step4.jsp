...
<style>
    ...
    <form action="step4_proc.jsp" method="post">
    	<fieldset>
    		<legend>연봉/복지(문화)</legend>
    		<table border="0">
    			<tr>
    				<td><label for='salary'>희망 연봉($)</label></td>
    				<td>
    					<select id="salary" name='salary'>
    						<option value="min">~$20,000</option>
    						<option value="s1">$20,000~$30,000</option>
    						<option value="s2">$30,000~$50,000</option>
    						<option value="s3">$50,000~$100,000</option>
    						<option value="max">$100,000~</option>
    					</select>
    				</td>
				</tr>
				<tr>    				
    				<td><label for='welfare'>희망 복지</label></td>
    				<td>
    					<textarea rows="4" cols="40" name='welfare'
    						placeholder="원하는 사내복지 및 문화사항 기재"
    					></textarea>
    				</td>
    			</tr>
    			<tr>
...
네이버 서비스와 관련하여 궁금하신 사항이 있으시면 고객센터(대표번호: 1588 – 3820/ 평일 09:00~18:00)로 문의 주시기 바랍니다.
    					...
    					<input type="checkbox" id="sla_agree" 
    					name="sla" value="yes">
    					<label id="sla_label" for='sla_agree'>
    					위 약관을 확인했으며, 동의합니다.</label>
    				</td>
    			</tr>
    			
    			<%@ include file="common/_resetsubmit.jsp" %>
    		</table>
    		<% if( errorCode != null && errorCode.equals("43") ) { %>
    		<script type="text/javascript">
    			alert("error 43!!!");
    			console.log("error 43!!!");
    			var slaLabel = document.getElementById('sla_label');
    			slaLabel.style.backgroundColor = 'red';
    		</script>
    		<% } %>
